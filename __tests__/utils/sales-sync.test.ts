...
describe('buildOrderItems', () => {
  it('maps local lines to Coop order items by SKU with computed line totals', () => {
    ...
    expect(buildOrderItems(items, skuMap)).toEqual([
      { product_id: 'ZMYFDFDRBEFLVR01', bundle_group: null, qty: 2, unit_price: 170, line_total: 340 },
      { product_id: 'ZMYFDMEATBEFWHL01', bundle_group: null, qty: 1, unit_price: 200, line_total: 200 },
    ]);
  });
  ...
  it('skips lines whose product has no SKU (unknown to Coop)', () => {
    ...
    expect(buildOrderItems(items, skuMap)).toEqual([
      { product_id: 'ZMYFDFDRBEFLVR01', bundle_group: null, qty: 1, unit_price: 170, line_total: 170 },
    ]);
  });

  it('carries a pick line\'s bundle group so Coop can tie it to its bundle header', () => {
    const items = [line({ productId: 1, price: 0, bundleGroup: '2' })];
    const skuMap = new Map([[1, 'ZMYFDFDRBEFLVR01']]);
    expect(buildOrderItems(items, skuMap)).toEqual([
      { product_id: 'ZMYFDFDRBEFLVR01', bundle_group: '2', qty: 1, unit_price: 0, line_total: 0 },
    ]);
  });
...
describe('buildBundleOrderItems', () => {
  ...
  it('emits one header line per cart bundle, carrying its price as the line total', () => {
    ...
    expect(buildBundleOrderItems(bundles, uuidByPreset)).toEqual([
      { product_id: null, bundle_id: '4abf67b0-4004-4d76-b687-e43cb7c0a6e6', bundle_group: '1', qty: 1, unit_price: 650, line_total: 650 },
      { product_id: null, bundle_id: 'bundle-buy-any-2', bundle_group: '2', qty: 1, unit_price: 550, line_total: 550 },
    ]);
  });

  it('still emits a header for an unresolvable bundle as a custom premium line (bundle_id null)', () => {
    ...
    // Every header keeps its 1-based index so it lines up with its own picks; an
    // unresolvable bundle records its price with a null bundle_id (custom bundle).
    expect(buildBundleOrderItems(bundles, uuidByPreset)).toEqual([
      { product_id: null, bundle_id: '4abf67b0-4004-4d76-b687-e43cb7c0a6e6', bundle_group: '1', qty: 1, unit_price: 650, line_total: 650 },
      { product_id: null, bundle_id: null, bundle_group: '2', qty: 1, unit_price: 650, line_total: 650 },
      { product_id: null, bundle_id: null, bundle_group: '3', qty: 1, unit_price: 650, line_total: 650 },
    ]);
  });
