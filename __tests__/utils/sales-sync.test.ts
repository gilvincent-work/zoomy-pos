...
describe('buildBundleOrderItems', () => {
  ...
  it('emits one bundle_id line per cart bundle, carrying its price as the line total', () => {
    ...
    expect(buildBundleOrderItems(bundles, uuidByPreset)).toEqual([
      { bundle_id: '4abf67b0-4004-4d76-b687-e43cb7c0a6e6', bundle_group: '1', qty: 1, unit_price: 650, line_total: 650 },
      { bundle_id: 'bundle-buy-any-2', bundle_group: '2', qty: 1, unit_price: 550, line_total: 550 },
    ]);
  });
  ...
  it('skips bundles with no resolvable Coop id (unsynced or ad-hoc), degrading to header-only', () => {
    ...
    // Header keeps its original 1-based index ('1') so it still lines up with the
    // picks buildInsertItems tagged for the same bundle.
    expect(buildBundleOrderItems(bundles, uuidByPreset)).toEqual([
      { bundle_id: '4abf67b0-4004-4d76-b687-e43cb7c0a6e6', bundle_group: '1', qty: 1, unit_price: 650, line_total: 650 },
    ]);
  });
